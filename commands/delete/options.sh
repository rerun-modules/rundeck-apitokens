...
# Created: Wed Dec 23 08:00:11 PST 2015
#
#/ usage: rundeck-apitokens:delete  --token-user <>  --url <$RUNDECK_URL> [ --username <$RUNDECK_USER>] [ --password <$RUNDECK_PASSWORD>] [ --apikey <$RUNDECK_APIKEY>]  --token-id <> 
...
rerun_options_parse() {
    ...
    do
        OPT="$1"
        case "$OPT" in
            --token-user) rerun_option_check $# $1; TOKEN_USER=$2 ; shift ;;
            ...
            --token-id) rerun_option_check $# $1; TOKEN_ID=$2 ; shift ;;
            # help option
    ...
    [[ -z "$TOKEN_USER" ]] && { echo >&2 "missing required option: --token-user" ; return 2 ; }
    [[ -z "$URL" ]] && { echo >&2 "missing required option: --url" ; return 2 ; }
    [[ -z "$TOKEN_ID" ]] && { echo >&2 "missing required option: --token-id" ; return 2 ; }
